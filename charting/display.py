...
def _chart_create(ratio, ratio_census, cancer_name, flag, title0, title1):
    ...
    if flag == 'g':
        plt.xlabel(f'Top {index_len} genes among samples of {cancer_name.replace("_", " ")} cancer with confirmed mutations (%) (extracted from CosmicMutantExportCensus.tsv)',        
    x = -0.04,fontsize = 12, labelpad=6)
        filename = f"output\{cancer_name}_cancer\charts\{cancer_name}_cancer"
    else:
        #plt.xlabel(f'Top {index_len} variants among samples of gene {title0} and {title1} ({cancer_name}) with confirmed mutations (%) (extracted from CosmicMutantExportCensus.tsv)',     
        plt.xlabel(f'Top {index_len} AA variants among samples of {cancer_name} cancer with confirmed mutations (%) (extracted from CosmicMutantExportCensus.tsv)', x = -0.04,fontsize = 12, labelpad=6)
